...
def Aptamers_SS(data, conv3):# data, conv3
...
if __name__ == "__main__":
    ...
    pros = data['Protein']
    apts = data['Aptamer']
    pros_ss = "./dataset/training/Proteins_SS.fas"
    
    conv1, conv2, conv3 = def_convs()
    
    # Protein 3Mer 
    pros_3Mer = Proteins_3Mer(pros, conv1)
    torch.save(pros_3Mer, './dataset/training/Proteins_3Mer.pt')
    
    # Protein PseAAC
    pros_PseAAC = Proteins_PseAAC(pros)
    torch.save(pros_PseAAC, './dataset/training/Proteins_PseAAC.pt')
    
    # Protein Secondary Structure
    pros_SS = Proteins_SS(pros_ss, conv2)
    torch.save(pros_SS, './dataset/training/Proteins_SS.pt')
    
    # Aptamer 3Mer
    apts_3Mer = Aptamers_3Mer(apts, conv1)
    torch.save(apts_3Mer, './dataset/training/Aptamers_3Mer.pt')
    
    # Aptamer PseKNC
    apts_PseKNC = Aptamers_PseKNC(apts)
    torch.save(apts_PseKNC, './dataset/training/Aptamers_PseKNC.pt')
    
    # Aptamer Secondary Structure
    apts_SS = Aptamers_SS(apts, conv3)
    torch.save(apts_SS, './dataset/training/Aptamers_SS.pt')
    
    # concatenate features
    Proteins_3Mer = torch.load('./dataset/training/Proteins_3Mer.pt')
    Proteins_PseAAC = torch.load('./dataset/training/Proteins_PseAAC.pt')
    Proteins_SS = torch.load('./dataset/training/Proteins_SS.pt')
    
    Aptamers_3Mer = torch.load('./dataset/training/Aptamers_3Mer.pt')
    Aptamers_PseKNC = torch.load('./dataset/training/Aptamers_PseKNC.pt')
    Aptamers_SS = torch.load('./dataset/training/Aptamers_SS.pt')
    
    features_tensor = torch.cat((Proteins_3Mer, Proteins_PseAAC, Proteins_SS, Aptamers_3Mer, Aptamers_PseKNC, Aptamers_SS), 1)
    torch.save(features_tensor, './dataset/training/features_tensor.pt')
    
    targets_tensor = torch.tensor(data['Label'], dtype=torch.int32)
    torch.save(targets_tensor, './dataset/training/targets_tensor.pt')
    
    # split train and test
    x_train, x_test, y_train, y_test = train_test_split(
    features_tensor, targets_tensor, test_size=580, random_state=42)
    
    torch.save(x_train, './dataset/training/x_train.pt')
    torch.save(x_test, './dataset/training/x_test.pt')
    torch.save(y_train, './dataset/training/y_train.pt')
    torch.save(y_test, './dataset/training/y_test.pt')
